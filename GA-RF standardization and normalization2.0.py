...
base_log_file = f"model_training_log—GA_RF_{preprocessing_tag}2.0.txt"
...
def visualize_data_distribution(X_before, X_after, method):
    ...
    plt.tight_layout()
    distribution_img = os.path.join("data explore", f"data_distribution_{method}2.0.png")
...
if processor is not None:
    processor_filename = f"{PREPROCESSING_METHOD}_processor2.0.joblib"
...
# 保存模型到文件
model_filename = f"ga_optimized_rf_model_{preprocessing_tag}2.0.joblib"
...
# 保存散点图到outputs文件夹
base_img_name = f"GA_RF_Prediction_{preprocessing_tag}2.0"
img_extension = ".png"
...
if __name__ == "__main__":
    ...
    new_prediction_data = pd.DataFrame({
        ...
        'aspect_ratio': [0.7, 0.8, 0.9, 1, 1.1, 1.2, 1.3, 1.4, 1.5]
    })
    ...
    for i, params in new_prediction_data.iterrows():
        print(f"参数组合 {i + 1}:")
        print(
            f"  温度: {params['printing_temperature']}°C, 进料速率: {params['feed_rate']}%, 打印速度: {params['printing_speed']}mm/s, 宽高比: {params['aspect_ratio']}")
