...
# 生成唯一的日志文件名
base_log_file = "mediation_effect_analysis_log.txt"
...
plt.tight_layout()
plt.savefig(os.path.join("outputs", "mediation_feature_importance.png"), dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join("outputs", "model_comparison_scatter.png"), dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join("outputs", "model_metrics_comparison.png"), dpi=300)
plt.show()

# 导出所有预测结果和平均值到Excel
output_file = get_unique_filename(os.path.join("prediction_results", "mediation_analysis_predictions.xlsx"))
