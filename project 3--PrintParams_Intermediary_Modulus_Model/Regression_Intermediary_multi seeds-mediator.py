...
def run_regression_experiment(seed):
    ...
    # 使用经典方法计算中介效应
    classical_mediation, classical_models = calculate_classical_mediation_effects(
        train_data, predictors, mediators, target)
    ...
    # 存储经典中介模型以便后续评估
    models['classical_total'] = classical_models['total_model']
    models['classical_direct'] = classical_models['direct_model']
    for m in mediators:
        models[f'classical_mediator_{m.lower()}'] = classical_models['mediator_models'][m]

    model_names['classical_total'] = 'Classical Total Effect Model'
    model_names['classical_direct'] = 'Classical Direct Effect Model'
    for m in mediators:
        model_names[f'classical_mediator_{m.lower()}'] = f'Classical Mediator Model ({m})'
    ...
    # 2. 直接效应模型
    key = 'classical_direct'
    model = models[key]
    y_pred_val = model.predict(val_data[predictors + mediators])
    y_true_val = val_data[target]
    y_pred_test = model.predict(test_data[predictors + mediators])
    y_true_test = test_data[target]
    results[key] = {
        'val': {
            'MSE': mean_squared_error(y_true_val, y_pred_val),
    ...
    # 3. 中介变量预测模型
    for mediator in mediators:
        key = f'classical_mediator_{mediator.lower()}'
        model = models[key]
        y_pred_val = model.predict(val_data[predictors])
        y_true_val = val_data[mediator]
        y_pred_test = model.predict(test_data[predictors])
        y_true_test = test_data[mediator]
        results[key] = {
            'val': {
                'MSE': mean_squared_error(y_true_val, y_pred_val),
                'R2': r2_score(y_true_val, y_pred_val),
    ...
    return results, model_names, val_data[target], test_data[target], classical_mediation
...
def calculate_average_classical_mediation(classical_mediation_list):
    """计算多次实验的经典中介效应平均值"""
    if not classical_mediation_list:
        return {}

    avg_mediation = {}
    predictors = [k for k in classical_mediation_list[0].keys() if k != 'overall']
